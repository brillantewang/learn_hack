...
- **Every exercise must have a corresponding HackTest file** as a sibling in the same unit folder, using CamelCase to match the class name (e.g., `Ex01BasicsTest.hack` with class `Ex01BasicsTest`). HackTest requires filenames to end in `Test.hack` and class names to match the filename. Create the test file at the same time as the exercise. Tests validate the learner's implementation; the `main()` entry point in the exercise is optional for quick manual runs. **Exception:** code-reading/predict-output exercises don't need test files — validation happens via discussion with the tutor. These exercises should be split into two files: the exercise file (snake_case, e.g., `ex03_predict.hack`) has bare code snippets with blank `/* YOUR ANSWER: ... */` blocks — no category labels, hints, or descriptions that give away the concept being tested. A separate CamelCase answers file (e.g., `Ex03PredictAnswers.hack`) contains the concept category, explanation, and correct answer for each snippet. After grading, append a `/* GRADING */` section at the bottom of the exercise file with score and per-snippet feedback.
